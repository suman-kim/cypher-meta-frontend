...
function TeamBlock({
  ...
  return (
    <div className="rounded-md border border-line bg-surface p-2">
      <div className={`mb-1.5 text-xs font-bold ${win ? "text-blue-300" : "text-red-300"}`}>
        {team.result ? (win ? "승리 팀" : "패배 팀") : `팀 ${index + 1}`}
      </div>
  ...
  return (
    <div
      className={`overflow-hidden rounded-lg border border-l-4 ${
        win ? "border-line border-l-win bg-win/10" : "border-line border-l-lose bg-lose/10"
      }`}
    >
      <button
        type="button"
        ...
        className={`flex w-full items-center gap-3 px-3 py-2.5 text-left transition-colors ${
          win ? "hover:bg-win/15" : "hover:bg-lose/15"
        }`}
      >
