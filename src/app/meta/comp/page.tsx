...
export default async function CompMetaPage() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black text-gray-50">조합 티어</h1>
          <p className="mt-1 text-sm text-gray-500">
            상위 랭커 매치의 팀 조합 집계 + 커뮤니티가 투표한 추천 5인 조합.
          </p>
        </div>
      </div>
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="text-lg font-bold text-gray-100">커뮤니티 추천 조합</h2>
          <span className="text-xs text-gray-500">
            유저 투표 {compVotes ? `· ${compVotes.totalBallots.toLocaleString()}표 · ${compVotes.distinctCombos.toLocaleString()}종` : ""}
          </span>
        </div>
